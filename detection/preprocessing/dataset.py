...
__all__ = ['Cinc2017Dataset', 'Cpsc2018Dataset']


class BaseDataset(ABC, Dataset):
    """
    The base dataset that all the other datasets derive from.
	Tập dữ liệu cơ sở có nghĩa là tất cả tất cả tập dữ liệu khác đều lấy từ đó.

    When implementing (thực hiện) your own dataset, you have to implement the `read_record` and `get_dataframe` functions.
    ...
    ## Constructor, hàm xây dựng luôn được gọi khi một đối tượng được tạo ra
    ...
    ## Hàm len dùng trả vể độ dài mẫu trong tập dữ liệu
    def __len__(self):
        """
    ...
    ## Hàm getitem cho phép lập chỉ mục bằng cách gọi hàm read_record, ngoài ra còn hỗ trợ kiểu lát cắt
    ...
    ## Hàm iter() được tích hợp sẵn trong Python, trả về một đối tượng iterator; dùng kết hợp với vòng lặp while, for
    def __iter__(self):
        return iter(self[i] for i in range(len(self)))

    ## Hàm repr() trả về một đối tượng có thể in được
    def __repr__(self):
        return self.name()
    ...
    ## Hàm exists() trả về True nếu path tồn tại, ngược lại trả về False
    def data_exist(self):
        """
    ...
    ## Hàm isfile kiểm tra xem đường dẫn có đến file hay không
    ...
    ## Dùng để tải xuống dữ liệu cơ sở
    def download(self):
        """
